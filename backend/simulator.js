            ...
            for (let i = 0; i < this.config.txs_per_client; i++) {
                ...
                if (type === 'READ') {
                    ...
                    if (accounts.length < 1) {
                        ...
                        ok = !!(res.data && res.data.success);
                        if (ok) this.stats.deposits_ok++;
                        else this.stats.last_error = res.data?.data?.message || res.data?.error || 'Deposit failed';
                    }
                } else {
                    if (accounts.length < 2) {
                        ...
                        if (from.account_no === to.account_no) {
                            ...
                            ok = !!(res.data && res.data.success);
                            if (ok) this.stats.transfers_ok++;
                            else this.stats.last_error = res.data?.data?.message || res.data?.error || 'Transfer failed';
                        }
                    }
                }

                if (ok) this.stats.transactions_ok++;
                else this.stats.transactions_failed++;
